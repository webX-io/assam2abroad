...
export default function AdminDashboard({ currentPath, onNavigate }: AdminDashboardProps) {
  ...
  // Real-time subscription to Enquiries in Firestore
  useEffect(() => {
    if (!user) return;

    const unsubscribe = listenToEnquiries((fetchedEnquiries) => {
      // Find the count of new (uncontacted) enquiries
      const newCount = fetchedEnquiries.filter(e => e.status === 'New').length;
      
      // If we already had a count, and it increased, show a notification
      if (prevNewCount !== null && newCount > prevNewCount) {
        setShowNotificationBadge(true);
        // Find the newest enquiry that was added
        const newest = fetchedEnquiries[0];
        if (newest) {
          setToastMessage(`New Enquiry from ${newest.fullName}!`);
  ...
  const handleToggleStatus = async (enquiry: Enquiry) => {
    const newStatus = enquiry.status === 'New' ? 'Contacted' : 'New';
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col md:flex-row text-slate-800 font-sans">
      ...
      `}>
        ...
        <div className="flex-1 px-4 py-2 space-y-1.5">
          <button className="w-full flex items-center space-x-3 bg-white/[0.06] border border-white/10 text-white font-bold text-sm py-3 px-4 rounded-xl shadow-sm">
      ...
      <div className="flex-1 overflow-y-auto min-h-screen p-6 sm:p-10 bg-slate-50 relative">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-200 pb-6 mb-8">
          <div>
            <h2 className="text-2xl font-bold text-slate-900 tracking-tight font-display">Student Enquiries</h2>
            <p className="text-xs text-slate-500 mt-1">Real-time repository of website submissions and admission leads</p>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 mb-8">
          ...
          <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-sm relative overflow-hidden flex items-center justify-between group hover:border-slate-300 transition duration-300">
            <div className="space-y-1">
              <span className="text-[11px] font-mono tracking-wider font-extrabold text-slate-400 uppercase">Total Enquiries</span>
              <div className="text-3xl font-extrabold text-slate-900">{stats.total}</div>
            </div>
            <div className="p-3 bg-slate-100 rounded-xl text-slate-600 group-hover:scale-110 transition duration-300">
          ...
          <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-sm relative overflow-hidden flex items-center justify-between group hover:border-slate-300 transition duration-300">
            <div className="space-y-1">
              <span className="text-[11px] font-mono tracking-wider font-extrabold text-slate-400 uppercase">New Enquiries</span>
              <div className="text-3xl font-extrabold text-slate-900 flex items-center gap-2">
                <span>{stats.newCount}</span>
                {stats.newCount > 0 && (
                  <span className="animate-pulse bg-emerald-500 h-2.5 w-2.5 rounded-full inline-block" />
                )}
              </div>
            </div>
            <div className="p-3 bg-amber-500/10 rounded-xl text-amber-500 group-hover:scale-110 transition duration-300 border border-amber-500/10">
              <Sparkles className="w-6 h-6" />
            </div>
            <div className="absolute top-0 bottom-0 left-0 w-1 bg-amber-400" />
          </div>

          {/* Card: Contacted Enquiries */}
          <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-sm relative overflow-hidden flex items-center justify-between group hover:border-slate-300 transition duration-300">
            <div className="space-y-1">
              <span className="text-[11px] font-mono tracking-wider font-extrabold text-slate-400 uppercase">Contacted Enquiries</span>
              <div className="text-3xl font-extrabold text-slate-900">{stats.contacted}</div>
            </div>
            <div className="p-3 bg-emerald-500/10 rounded-xl text-emerald-500 group-hover:scale-110 transition duration-300 border border-emerald-500/10">
              <CheckSquare className="w-6 h-6" />
            </div>
            <div className="absolute top-0 bottom-0 left-0 w-1 bg-emerald-400" />
          </div>
        </div>

        {/* CONTROLS (SEARCH, FILTER & SORT) */}
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 mb-6 flex flex-col md:flex-row gap-4 justify-between items-center">
          {/* Search Bar */}
            ...
            >
              ...
              <div className="p-6 border-t border-slate-200 bg-slate-50 flex gap-3">
                <button
                  ...
                  className={`flex-1 font-bold text-xs uppercase tracking-wider py-3.5 rounded-xl border transition flex items-center justify-center space-x-2 cursor-pointer ${
                ...
                >
                  ...
                  <span>{selectedEnquiry.status === 'New' ? 'Mark Contacted' : 'Mark New'}</span>
                </button>

                <button
                  onClick={() => handleDeleteEnquiry(selectedEnquiry.id)}
                  className="bg-rose-50 hover:bg-rose-100 border border-rose-200/60 text-rose-600 font-bold text-xs uppercase tracking-wider py-3.5 px-4 rounded-xl transition flex items-center justify-center space-x-1.5 cursor-pointer"
                  title="Delete lead"
                >
